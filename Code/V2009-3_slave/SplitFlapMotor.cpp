...
SplitFlapMotor::SplitFlapMotor(int in1, int in2, int in3, int in4, int hallPin)
  : stepper(AccelStepper::HALF4WIRE, in1, in3, in2, in4),
    ...
    magnetPassCount(0),
    motorBusy(false),
    moduleListeningFromLeft(false)  // <<< FIXED: moved inside initializer list
{}
...
void SplitFlapMotor::begin() {
    ...
    config.begin();
    zeroOffsetDegrees = config.getZeroOffset();
    stepOffset = config.getStepOffset();
}
...
void SplitFlapMotor::resetConfig() {
  Serial.println("-------------------------------------------");
  Serial.println("Resetting configuration to defaults...");

  config.setZeroOffset(91);
  config.setStepOffset(0);
...
void SplitFlapMotor::setStepOffset(long offset) {
  ...
  stepOffset = offset;
  config.setStepOffset(offset);
  Serial.print("New Offset: ");
  Serial.println(stepOffset);
}

void SplitFlapMotor::setZeroOffsetSteps(long steps) {
  const float stepsPerDegree = 4096.0 / 360.0;
  float newOffsetDegrees = steps / stepsPerDegree;

  Serial.println("-------------------------------------------");
  Serial.print("Previous Zero Offset (steps): ");
  Serial.println((int)(zeroOffsetDegrees * stepsPerDegree));
  Serial.print("Previous Zero Offset (degrees): ");
  Serial.println(zeroOffsetDegrees);

  zeroOffsetDegrees = newOffsetDegrees;
  config.setZeroOffset(zeroOffsetDegrees);

  Serial.print("New Zero Offset (steps): ");
  Serial.println(steps);
  Serial.print("New Zero Offset (degrees): ");
  Serial.println(zeroOffsetDegrees);
}

void SplitFlapMotor::setZeroOffsetStepsInteractive() {
  if (motorBusy) {
    Serial.println("Cannot change zero offset while motor is moving.");
    return;
  }

  const float stepsPerRevolution = 4096.0;
  const float stepsPerDegree = stepsPerRevolution / 360.0;
  const int numFlaps = NUM_FLAPS;
  const float stepsPerFlap = stepsPerRevolution / numFlaps;

  Serial.println("-------------------------------------------");
  Serial.print("Current Zero Offset (steps): ");
  Serial.println((int)(zeroOffsetDegrees * stepsPerDegree));
  Serial.print("Which is roughly: ");
  Serial.print(zeroOffsetDegrees);
  Serial.println(" degrees.");
  Serial.print("At ");
  Serial.print(stepsPerFlap);
  Serial.println(" steps per flap index.");
  ...
  if (response == 'y' || response == 'Y') {
    Serial.println("Enter new zero offset (in steps, integer):");
    while (Serial.available() == 0);
    String line = Serial.readStringUntil('\n');
    line.trim();
    int offsetSteps = line.toInt();
    zeroOffsetDegrees = offsetSteps / stepsPerDegree;
    config.setZeroOffset(zeroOffsetDegrees);
    Serial.print("New Zero Offset (steps): ");
    Serial.println(offsetSteps);
    Serial.print("Which is roughly: ");
    Serial.print(zeroOffsetDegrees);
    Serial.println(" degrees.");
  } else {
...
void SplitFlapMotor::setStepOffsetInteractive() {
  ...
  if (response == 'y' || response == 'Y') {
    ...
    config.setStepOffset(stepOffset);
...
void SplitFlapMotor::printHelp() {
  ...
  Serial.println("***POS        - Print current step and degree position");
  ...
  Serial.println("***MOVE <N>   - Move forward by N steps");
  Serial.println("***ZOFFSET    - Set magnet-to-flap offset in degrees");
  Serial.println("***OFFSET     - Set global step offset");
  Serial.println("***RESETCONFIG - Reset stored config to default values");
  Serial.println("***HELP       - Show this help message");

  Serial.println("***LEFTMODE   - Enable left-module input mode");
  Serial.println("****[message] - Inject paragraph_message (for dev testing)");
  Serial.println();
...
void SplitFlapMotor::setModuleListening(bool enabled) {
    moduleListeningFromLeft = enabled;
    Serial.print("Module listening from left RX set to: ");
    Serial.println(enabled ? "TRUE" : "FALSE");
}

bool SplitFlapMotor::isModuleListeningFromLeft() {
    return moduleListeningFromLeft;
}
