...
def build_paragraph_message(chars, ids):
    ...
    chars = (' ' * left_padding) + chars + (' ' * right_padding)  # Center-align with spaces
...
def main():
    ...
    try:
        with open(CSV_FILE, newline='') as csvfile, serial.Serial(PORT, BAUD_RATE, timeout=2) as ser:
            ...
            for i, delay in enumerate([5, 10, 15]):
                print(f"  Sending ZERO command {i+1}/3 with {delay}s delay")
            ...
            for i, row in enumerate(reader):
                ...
                msg_text = row[0]  # Preserve spaces, do not strip
                try:
                ...
                if msg_text == "":
                    print("  Skipped: Empty message (not even a space)")
                    continue

                msg_text = msg_text[:9]  # Enforce 9-char limit
                msg = build_paragraph_message(msg_text, MODULE_IDS)
                print(f"  Sending: {repr(msg_text)} | Duration: {duration}s")
                print(f"  Built Msg: {repr(msg)}")
